              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-4 px-3 py-3 rounded-xl text-sm font-semibold transition-all ${
                    isActive ? "bg-[#fff0f3] text-[#ba0036]" : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"
                  }`}
                >
        ...
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-8 shrink-0 relative z-30">
          <div className="flex items-center gap-3">
            ...
            <div className="relative w-full max-w-[200px] sm:max-w-xs lg:max-w-md hidden sm:block">
              ...
              <input type="text" placeholder="Search experiences, stays..." className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-full text-sm outline-none focus:border-[#ba0036]/30 transition-all"/>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-2.5 bg-gray-50 border border-gray-100 py-1.5 pl-1.5 sm:pl-3 pr-1.5 rounded-full cursor-pointer hover:border-gray-200 transition-all ml-1">
              {customerData.isLoading ? (
                ...
                <>
                  <span className="text-xs font-bold text-gray-800 hidden sm:block truncate max-w-[120px]">{customerData.name}</span>
                  {customerData.avatar ? (
                    <img src={customerData.avatar} alt="Profile" className="w-8 h-8 rounded-full object-cover ring-1 ring-gray-200" />
                  ) : (
